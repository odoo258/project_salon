...
class PagforExport(models.Model):
    _name = 'pagfor.export'

    def generate_pagfor_remessa(self):

        lot_payment_itau = []
        lot_payment_bradesco = []
        lot_payment_santander = []
        lot_payment_bbrasil = []
        lot_payment_siccob = []

        for rec_lot_payment in self.env['lot.payment'].browse(self._context['active_ids']):

            if rec_lot_payment.status in ('paid', 'canceled'):
                continue

            # msg = self.validate(rec_lot_payment)
            # if msg:
            #     raise UserError(_('Error: %s') %msg)

            bank_code = rec_lot_payment.payment_mode_id.bank_account_id.bank_id.bic

            if bank_code == '033':  # Santander
                lot_payment_santander.append(rec_lot_payment)
        ...
        if lot_payment_santander: # Santander
            self.env['pagfor.remessa.santander'].gerar_lot_pagfor_remessa_santander(lot_payment_santander)
