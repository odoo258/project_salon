...
class LotPayment(models.Model):
    _name = 'lot.payment'

    @api.multi
    def unlink(self):

        for lot_payment in self:
            if lot_payment.status == 'paid':
                raise UserError(_('Error! Is not possible delete bank payment(s) paid!'))

        return super(LotPayment, self).unlink()
    ...
    invoice_installment_ids = fields.One2many('account.move.line', 'lot_payment_id', string='Invoice Installments', readonly=True)
    status = fields.Selection([('open', 'Open'), ('confirmed', 'Confirmed'), ('paid', 'Paid')], string='Status', default='open', readonly=True)