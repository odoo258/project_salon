odoo.define("pos_pricing.pricelist", function (require) {
    ...
    pos_model.Order = pos_model.Order.extend({
        ...
        // this function returns the new price after checking the computation method of the pricelist item
        set_discount: function (line, item) {
            var new_discount = 0;
            switch (item.compute_price){
                case 'fixed': new_discount = 0; break;
                case 'percentage': new_discount = item.percent_price; break;
            }
            return new_price;
        },
                    ...
                    for(var j in product_items){
                        if(product_items[j].product_tmpl_id[0] == lines[l].product.product_tmpl_id){
                           if(lines[l].quantity >= product_items[j].min_quantity){
                           		if(product_items[j].percent_price > discount){
						   			discount = product_items[j].percent_price;
									w_item = product_items[j];
								}
                            }
                        }
                    }
                    ...
                    for(var j in category_items){
                        if(category_items[j].categ_id[0] == lines[l].product.pos_categ_id[0]){
                           if(lines[l].quantity >= category_items[j].min_quantity){
							   if(category_items[j].percent_price > discount){
						   			discount = category_items[j].percent_price;
									w_item = category_items[j];
							   }
                           }
                        }
                    }
...
//                if there are no rules set for product or category, we will check global pricelists
                if(global_items.length > 0){
                    for(var j in global_items){
                        if(lines[l].quantity >= global_items[j].min_quantity){
                            if(global_items[j].percent_price > discount){
						   		discount = global_items[j].percent_price;
								w_item = global_items[j];
							}
                        }
                    }
                }
				if(w_item != 0){
					if(temp < 0){
                    	temp = lines[l].quantity - w_item.min_quantity;
                        new_price = self.set_price(lines[l], w_item);
                        new_discount = w_item.percent_price;
                    }
                    else if(temp > (lines[l].quantity - w_item.min_quantity) &&
                    		(lines[l].quantity - w_item.min_quantity) >= 0){
                    	temp = lines[l].quantity - w_item.min_quantity;
                    	new_price = self.set_price(lines[l], w_item);
                   		new_discount = w_item.percent_price;
                    }
					lines[l].set_discount(new_discount);
				}
                else{
                    lines[l].set_discount(lines[l].product.percent_price);
                }
            }
        },
        ...
        set_client: function(client){
            ...
            if(client){
               ...
               self.apply_pricelist(null, null);
            }
