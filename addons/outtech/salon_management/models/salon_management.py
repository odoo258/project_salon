...
class SalonChair(models.Model):
    ...
    def collection_today_updater(self):
       ...
       for values in self.search([]):
            invoiced_records = self.search([('stage_id', 'in', [3, 4]),('chair_id', '=', values.id)])
            total = 0
...
class SalonOrder(models.Model):
    ...
    partner_id = fields.Many2one(
        ...
        help="If the customer is a regular customer, then you can add the customer in your database"
    )
    ...
    stage_id = fields.Many2one('salon.stages', string="Stages")
...
class SalonOrderLine(models.Model):
    ...
    service_id = fields.Many2one(
        'product.template', string="Service", domain=[('type', '=', 'service')]
    )
    ...
    def onchange_service(self):
        ...
        self.time_taken = 0  # self.service_id.time_taken
