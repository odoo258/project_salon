...
models.load_fields('res.company','cnpj_cpf');
models.load_fields('res.company','street');
models.load_fields('res.company','street2');
models.load_fields('res.company','number');
...
models.Orderline = models.Orderline.extend({
    ...
    init_from_JSON: function(json) {
        if (!json['pack_lot_lines']){
            json.pack_lot_ids = false
        }
...
//        }
    ...
    get_base_price:    function(){
        ...
        return Math.floor(parseFloat((this.get_unit_price() - (this.get_unit_price() * this.get_discount() / 100)) * 100).toFixed(2)) / 100;
    },
    get_all_prices: function(){
       ...
       if(this.discount_fixed > 0)
       {
            ...
            var price_unit = Math.floor(parseFloat((this.get_unit_price() - (this.get_unit_price() * this.get_discount() / 100)) * 100).toFixed(2)) / 100;
            }
        var taxtotal = 0;
        ...
        if(this.discount_fixed > 1) {
            all_taxes = this.compute_all(product_taxes, price_unit, 1, this.pos.currency.rounding);
        }
        else{
            all_taxes = this.compute_all(product_taxes, price_unit, this.get_quantity(), this.pos.currency.rounding);
        }
...
models.Order = models.Order.extend({
    ...
    export_for_printing: function(){
        ...
        var receipt = {
            ...
            company:{
                ...
                address: company.street + ', ' + company.number + ' ' + company.street2 + ' - ' + company.zip
            },
            shop:{
                name: shop.name,
            },
    ...
    get_total_without_tax: function() {
        ...
        if (total_fixed_disc) {
         ...
         if (total_percent_disc) {
            ...
            return Math.floor((temp - (temp * total_percent_disc / 100)) * 100) / 100;
         }
        return round_pr(this.orderlines.reduce((function(sum, orderLine) {
            return sum + orderLine.get_price_without_tax();
        }), 0), this.pos.currency.rounding);
    ...
    get_subtotal_ext : function(){
            return round_pr(this.orderlines.reduce((function(sum, orderLine){
                return sum + ((orderLine.price * orderLine.quantity) - (((orderLine.price * orderLine.quantity) * orderLine.discount) / 100)) - orderLine.discount_fixed;
            }), 0), this.pos.currency.rounding);
        },
});   
