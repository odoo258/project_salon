...
class NFeSerializer(object):
    ...
    def deserialize(self, nfe):
        ...
        try:
            ...
            ref = self.env['br_account.document.related'].create(
                nfe_references)
        ...
        #TODO verifcar busca de carrier da nota
    ...
    def _get_nfe_identification(self):
        ...
        #
        res = {}
        fiscal_doc_ids = self.env['br_account.fiscal.document'].search(
        ...
        res['vendor_number'] = self.nfe.infNFe.ide.nNF.valor
    ...
    def _get_nfe_references(self):
        ...
        fiscal_doc_obj = self.env['br_account.fiscal.document']
        ...
        if self.nfref.refNF.CNPJ.valor:
            ...
            fiscal_document_ids = \
                self.env['br_account.fiscal.document'].search(
    ...
    def _get_details(self):
        ...
        fc_id = self.env['product.fiscal.classification'].search(
            [('name', '=', ncm)]
        )
        ...
        cfop = self.env['br_account.cfop'].search([('code','=',self.det.prod.CFOP.valor)])
        if cfop:
            inv_line['cfop_id'] = cfop.id
        ...
        inv_line['price_unit'] = float(float(self.det.prod.vUnCom.valor) + (float(self.det.prod.vDesc.valor) / float(self.det.prod.qCom.valor)))
        inv_line['price_gross'] = float(float(self.det.prod.vProd.valor) + float(self.det.prod.vDesc.valor))
        inv_line['valor_frete'] = float(self.det.prod.vFrete.valor)
        inv_line['valor_seguro'] = float(self.det.prod.vSeg.valor)
        inv_line['valor_desconto'] = float(self.det.prod.vDesc.valor)
        inv_line['discount'] = '%.2f' % (float(self.det.prod.vDesc.valor) / (inv_line['price_unit'] * inv_line['quantity']) * 100)
        inv_line['outras_despesas'] = float(self.det.prod.vOutro.valor)
        ...
        if not self.det.imposto.ISSQN.cListServ.valor:
            ...
            icms_ids = self.env['account.tax'].search(
                [('amount', '=', float(self.det.imposto.ICMS.pCredSN.valor)),
                 ('domain', '=', 'icms')])

            if "ICMSSN" in self.det.imposto.ICMS.nome_tag:  # Simples
                inv_line['icms_aliquota'] = self.det.imposto.ICMS.pCredSN.valor
                inv_line['icms_valor'] = self.det.imposto.ICMS.vCredICMSSN.valor
                inv_line['icms_csosn_simples'] = self.det.imposto.ICMS.CSOSN.valor
            else:
                inv_line['icms_aliquota'] = self.det.imposto.ICMS.pICMS.valor
                inv_line['icms_valor'] = self.det.imposto.ICMS.vICMS.valor
                inv_line['icms_cst_normal'] = cst#icms_ids[0].id if icms_ids else False
            if self.det.imposto.ICMS.modBC.valor:
                inv_line['icms_tipo_base'] = '%s' % self.det.imposto.ICMS.modBC.valor
            inv_line['icms_base_calculo'] = self.det.imposto.ICMS.vBC.valor
            inv_line[
                'icms_aliquota_reducao_base'] = self.det.imposto.ICMS.pRedBC.valor

            tax_icms = self.env['account.tax'].search([
                ('domain', '=', 'icms'), ('type_tax_use', '=', 'purchase'),
                ('amount', '=', (inv_line['icms_aliquota'] / Decimal(100)))
            ])
            ...
            # # ICMS ST
            #
            inv_line['icms_st_tipo_base'] = str(self.det.imposto.ICMS.modBCST.valor)
            inv_line['icms_st_aliquota_mva'] = self.det.imposto.ICMS.pMVAST.valor
            inv_line['icms_st_aliquota_reducao_base'] = self.det.imposto.ICMS.pRedBCST.valor
            inv_line['icms_st_base_calculo'] = self.det.imposto.ICMS.vBCST.valor
            inv_line['icms_st_aliquota'] = self.det.imposto.ICMS.pICMSST.valor
            inv_line['icms_st_valor'] = self.det.imposto.ICMS.vICMSST.valor
            ...
            if self.det.imposto.IPI.vBC.valor and \
                ...
                inv_line['ipi_tipo'] = 'percent'
                inv_line['ipi_base_calculo'] = self.det.imposto.IPI.vBC.valor
                inv_line['ipi_aliquota'] = self.det.imposto.IPI.pIPI.valor
                inv_line['ipi_cst'] = self.det.imposto.IPI.CST.valor#ipi_ids[0].id if ipi_ids else False

            elif self.det.imposto.IPI.qUnid.valor and \
                    self.det.imposto.IPI.vUnid.valor:
                inv_line['ipi_aliquota'] = self.det.imposto.IPI.vUnid.valor
                ...
                #      ('domain', '=', 'ipi')])
                inv_line['ipi_tipo'] = 'percent'
                inv_line['ipi_aliquota'] = Decimal(0)
                inv_line['ipi_cst'] = '49'#ipi_ids[0].id if ipi_ids else False

            inv_line['ipi_valor'] = self.det.imposto.IPI.vIPI.valor

            tax_ipi = self.env['account.tax'].search([
                ('domain', '=', 'ipi'), ('type_tax_use', '=', 'purchase'),
                ('amount', '=', (inv_line['ipi_aliquota'] / Decimal(100)))
            ])
            ...
            # # ISSQN
            #
            inv_line['issqn_base_calculo'] = self.det.imposto.ISSQN.vBC.valor
            inv_line['issqn_aliquota'] = self.det.imposto.ISSQN.vAliq.valor
            inv_line['issqn_valor'] = self.det.imposto.ISSQN.vISSQN.valor
            inv_line['issqn_tipo'] = self.det.imposto.ISSQN.cSitTrib.valor
        ...
        inv_line['pis_cst'] = self.det.imposto.PIS.CST.valor#pis_cst_ids[0].id if pis_cst_ids else False
        inv_line['pis_base_calculo'] = self.det.imposto.PIS.vBC.valor
        inv_line['pis_aliquota'] = self.det.imposto.PIS.pPIS.valor
        inv_line['pis_valor'] = self.det.imposto.PIS.vPIS.valor

        tax_pis = self.env['account.tax'].search([
            ('domain', '=', 'pis'), ('type_tax_use', '=', 'purchase'),
            ('amount', '=', (inv_line['pis_aliquota'] / Decimal(100)))
        ])
        if tax_pis and tax_pis[0].amount:
            inv_line['invoice_line_tax_id'].append((4, tax_pis[0].id, 0))

        # PISST
        inv_line['pis_st_base_caluclo'] = self.det.imposto.PISST.vBC.valor
        inv_line['pis_st_aliquota'] = self.det.imposto.PISST.pPIS.valor
        inv_line['pis_st_valor'] = self.det.imposto.PISST.vPIS.valor
        ...
        inv_line['cofins_cst'] = self.det.imposto.COFINS.CST.valor#cofins_cst_ids[0].id if cofins_cst_ids else False
        inv_line['cofins_base_calculo'] = self.det.imposto.COFINS.vBC.valor
        inv_line['cofins_aliquota'] = self.det.imposto.COFINS.pCOFINS.valor
        inv_line['cofins_valor'] = self.det.imposto.COFINS.vCOFINS.valor

        tax_cofins = self.env['account.tax'].search([
            ('domain', '=', 'cofins'), ('type_tax_use',
                                        '=', 'purchase'),
            ('amount', '=', (inv_line['cofins_aliquota'] / Decimal(100)))
        ])
        if tax_cofins and tax_cofins[0].amount:
            inv_line['invoice_line_tax_id'].append((4, tax_cofins[0].id, 0))

        # COFINSST
        inv_line['cofins_st_base_calculo'] = self.det.imposto.COFINSST.vBC.valor
        inv_line['cofins_st_aliquota'] = self.det.imposto.COFINSST.pCOFINS.valor
        inv_line['cofins_st_valor'] = self.det.imposto.COFINSST.vCOFINS.valor
    ...
    def _get_carrier_data(self):
        ...
        try:
            vehicle_ids = self.env['br_delivery.carrier.vehicle'].search(
                [('plate', '=', placa)])
    ...
    def _get_total(self):
        ...
        total = {
            ...
            'total_bruto': float(self.nfe.infNFe.total.ICMSTot.vProd.valor),
            'total_frete': float(self.nfe.infNFe.total.ICMSTot.vFrete.valor),
            'total_seguro': float(self.nfe.infNFe.total.ICMSTot.vSeg.valor),
            'total_desconto': float(self.nfe.infNFe.total.ICMSTot.vDesc.valor),
