...
class StockReportXls(ReportXlsx):
    ...
    def get_warehouse(self, data):
        if data.get('form', False) and data['form'].get('warehouse', False):
            l1 = []
            l2 = []
            loc = self.env['stock.location'].search([('id', '=', data['form']['location_id'])])
            obj = self.env['stock.warehouse'].search([('id', '=', data['form']['warehouse'])])
            name = '%s/%s' % (obj.name,loc.name)
            l1.append(name)
            l2.append(obj.id)
        return l1, l2
    ...
    def get_lines(self, data, warehouse, location, date):
        lines = []
        ...
        for obj in stock_history:
            ...
            if date:
                if location:
                    quant = product._compute_quantities_dict_by_location([location],False,False, False, False, False, date)
                else:
                    location_ids = []
                    company = self.env['stock.warehouse'].browse(warehouse).company_id.id
                    locations = self.env['stock.location'].search([('usage','=','internal'),('company_id','=',company)])
                    for l in locations:
                        location_ids.append(l.id)
                    quant = product._compute_quantities_dict_by_location(location_ids, False,False, False, False, False, date)
            else:
                if location:
                    quant = product._compute_quantities_dict_by_location([location],False,False, False, False, False, False)
                else:
                    location_ids = []
                    company = self.env['stock.warehouse'].browse(warehouse).company_id.id
                    locations = self.env['stock.location'].search([('usage','=','internal'),('company_id','=',company)])
                    for l in locations:
                        location_ids.append(l.id)
                    quant = product._compute_quantities_dict_by_location(location_ids, False, False, False, False, False, False)


            available_qty = quant[product.id]['virtual_available'] + \
                            quant[product.id]['outgoing_qty'] - \
                            quant[product.id]['incoming_qty']
            ...
            vals = {
                ...
                'virtual': quant[product.id]['virtual_available'],#product.with_context({'warehouse': warehouse}).virtual_available,
                'incoming': quant[product.id]['incoming_qty'],#product.with_context({'warehouse': warehouse}).incoming_qty,
                'outgoing': quant[product.id]['outgoing_qty'],#product.with_context({'warehouse': warehouse}).outgoing_qty,
                'net_on_hand': quant[product.id]['qty_available'],#product.with_context({'warehouse': warehouse}).qty_available,
                'total_value': value,
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        count = len(get_warehouse[0]) * 7 + 6
        sheet = workbook.add_worksheet(u'Informação de Estoque')
        ...
        if data['form']['date']:
            sheet.merge_range('A3:G3', 'Data em: ' + str(data['form']['date'][8:] + '/' + data['form']['date'][5:-3] + '/' + data['form']['date'][:-6]), format1)#str(datetime.datetime.now(pytz.timezone('America/Sao_Paulo')).strftime("%d-%m-%Y %H:%M %p")), format1)
        else:
            sheet.merge_range('A3:G3', 'Data em: ' + str(datetime.datetime.now(pytz.timezone('America/Sao_Paulo')).strftime("%d-%m-%Y %H:%M %p")), format1)
        sheet.merge_range(2, 7, 2, count, 'Armazem/Local', format1)
        sheet.merge_range('A4:G4', u'Informção de Produtos', format11)
        w_col_no = 6
        w_col_no1 = 7
        ...
        sheet.merge_range(4, 1, 4, 3, 'Nome', format21)
        sheet.merge_range(4, 4, 4, 5, u'Categoria', format21)
        sheet.write(4, 6, u'Preço de Custo', format21)
        p_col_no1 = 7
        for i in get_warehouse[0]:
            sheet.write(4, p_col_no1, u'Disponivel', format21)
            sheet.write(4, p_col_no1 + 1, u'Previsto', format21)
            sheet.write(4, p_col_no1 + 2, u'Entrada', format21)
            sheet.write(4, p_col_no1 + 3, u'Saída', format21)
            sheet.merge_range(4, p_col_no1 + 4, 4, p_col_no1 + 5, u'Qtd Fisíca', format21)
            # sheet.merge_range(4, p_col_no1 + 6, 4, p_col_no1 + 7, 'Total Vendas', format21)
            # sheet.merge_range(4, p_col_no1 + 8, 4, p_col_no1 + 9, 'Total Compras', format21)
            sheet.write(4, p_col_no1 + 6, u'Valor do Estoque', format21)
            p_col_no1 = p_col_no1 + 11
        prod_row = 5
        prod_col = 0
        for i in get_warehouse[1]:
            get_line = self.get_lines(data, i, data['form']['location_id'],data['form']['date'])
        ...
        for i in get_warehouse[1]:
            ...
            for each in get_line:
                ...
                if each['total_value'] < 0:
                    sheet.write(prod_row, prod_col + 6, each['total_value'], red_mark)
                else:
                    sheet.write(prod_row, prod_col + 6, each['total_value'], font_size_8)
                prod_row = prod_row + 1
            prod_row = 5
