...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_amount(self):
        for line in self:
            ...
            #     discount = (valor_bruto * (line.discount / 100))
            #price = line.price_unit
            price_value = line.price_unit
            if line.discount:
                price_value  = line.price_unit - (line.price_unit * (line.discount / 100))
            price = line.price_unit
            ctx = line._prepare_tax_context()
            tax_ids = line.taxes_id.with_context(**ctx)
            taxes = tax_ids.compute_all(
                price_value, line.order_id.currency_id,
    ...
    def _get_stock_move_price_unit(self):
        price_value = self.price_unit
        if self.discount:
            price_value  = self.price_unit - (self.price_unit * (self.discount / 100))
        price = self.price_unit

        ctx = self._prepare_tax_context()
        tax_ids = self.taxes_id.with_context(**ctx)
        taxes = tax_ids.compute_all(
            price_value, self.order_id.currency_id,
            self.product_qty, product=self.product_id,
            partner=self.order_id.partner_id)

        price = ((taxes['total_included'] / self.product_qty) + (self.valor_frete / self.product_qty)\
                + (self.valor_seguro / self.product_qty) + (self.outras_despesas / self.product_qty))
        ...
        #self.price_uinit = original_unit_price
        return price
